...
test('medium-risk tasks require independent human-reviewed evidence', () => {
  const repo = tempRepo();
  run(['init', repo]);
  run(['task', 'add', repo, '--id', 'T-2', '--risk', 'medium', '--outcome', 'Risky thing', '--gate', 'tests:node -e "process.exit(0)"', '--gate', 'independent-review:']);
  run(['gate', repo, 'T-2']);
...
test('local traces redact credentials and are ignored by git', () => {
  ...
  assert.doesNotMatch(event, /abc123|hunter2|topsecret/);
...
test('cold-session state is reconstructable by a second process', () => {
  ...
  run(['task', 'set', repo, '--id', 'T-3', '--blocker', 'Need the API contract']);
  run(['checkpoint', repo]);
