...
function renderKickoff(repo, state) {
  ...
  const activeGates = active?.gates.map((gate) => freshGate(gate, repo, state, active)) || [];
  let selected;
  try { selected = contextPacket(repo, state, active, 24000); }
  catch { selected = { records: [], rules: [], attempts: [] }; }
  const decisionText = selected.records.filter(r => r.type === 'decisions').map((item) => typeof item === 'string' ? item : `${item.id}: ${item.title || item.text}`).join(', ');
  const invariantText = selected.records.filter(r => r.type === 'invariants').map((item) => typeof item === 'string' ? item : `${item.id}: ${item.text}`).join('; ');
  const assumptionText = selected.records.filter(r => r.type === 'assumptions').map((item) => typeof item === 'string' ? item : `${item.id}: ${item.text} [${item.status || 'open'}]`).join('; ');
  const lines = [
    ...
    '## Completed work',
    '',
    ...(completed.length ? completed.slice(-10).map((task) => `- ${task.id}: ${task.outcome} · proof: ${task.gates.map((gate) => gate.evidence?.path || `${gate.id}:${gate.status}`).join(', ') || 'no gates'}`) : ['- none']),
    '',
    ...
    `- knowledge: ${state.knowledge.length ? selected.records.filter(r => r.type === 'knowledge').map((item) => `${item.id}: ${item.title}`).join(', ') : 'none recorded'}`,
    '',
    '## Continuation', '',
    `Use genesis context <repo> for bounded task context; ${selected.omitted} records omitted. Fetch full records with --id.`,
    ...selected.attempts.map(a => `- attempt ${a.id}: ${a.status}; ${a.stop_reason || 'no failure'}; input ${a.before_hash}`),
    ...selected.rules.map(r => `- active rule ${r.id}: ${r.rule}`),
    'Reuse intact fresh proof with genesis gate <repo> --reuse; historical evidence is not automatically fresh.', '',
    '## Resume',
    '',
...
function renderDashboard(repo, state) {
  ...
  const tasks = state.tasks.map((task) => taskSummary(task, repo, state));
...
function contextPacket(repo, state, task, budget = 12000) {
  const scopes = task?.scope || [], tags = new Set(task?.requirements || []);
  const packet = { task: task ? { id: task.id, outcome: task.outcome, state: task.state, scope: scopes, requirements: task.requirements,
  ...
  function add(key, item) {
    ...
    if (Buffer.byteLength(JSON.stringify(packet)) > budget - 20) { packet[key].pop(); packet.omitted++; }
  }
  if (Buffer.byteLength(JSON.stringify(packet)) > budget) throw new Error('context budget too small for task and gate contract; increase --bytes');
  for (const rule of state.learning_proposals.filter(r => r.status === 'active')) add('rules', { id: rule.id, rule: rule.rule, experiment: rule.experiment, approved_by: rule.approved_by });
  for (const grant of state.authorizations.filter(a => task && a.task === task.id)) add('authorizations', { id: grant.id, command: grant.command, usable: Boolean(validAuthorization(state, task, grant.id)), expires_at: grant.expires_at });
  for (const attempt of state.attempts.filter(a => a.task === task?.id).slice(-3)) add('attempts', { id: attempt.id, kind: attempt.kind, status: attempt.status, stop_reason: attempt.stop_reason, started_at: attempt.started_at, ended_at: attempt.ended_at, before_hash: attempt.before_hash });
  for (const type of ['invariants', 'decisions', 'knowledge', 'assumptions']) {
    for (const record of state[type] || []) {
      if (typeof record !== 'object' || record.status === 'superseded') continue;
      const matches = !record.paths?.length && !record.tags?.length || record.paths?.some(path => scopes.some(scope => path === scope || path.startsWith(scope + '/') || scope.startsWith(path + '/'))) || record.tags?.some(tag => tags.has(tag));
      if (matches) add('records', { type, ...record, included_because: record.paths?.length ? 'task scope' : record.tags?.length ? 'requirement tag' : 'project context' });
    }
  }
  ...
  if (task && existsSync(graphPath)) {
    ...
    for (const edge of graph.edges.filter(e => seeds.has(e.source) || seeds.has(e.target))) add('graph', { ...edge, index_source_hash: graph.sourceHash, advisory: true });
  }
  return packet;
}
...
function commandContext(parsed) {
  ...
  if (parsed.options.id) {
    const record = ['decisions', 'knowledge', 'invariants', 'assumptions', 'incidents'].flatMap(key => state[key]).find(r => r.id === parsed.options.id);
    if (!record) throw new Error('unknown context record');
    console.log(JSON.stringify(record, null, 2)); return;
  }
  const task = state.tasks.find(t => t.id === (parsed.positional[1] || state.lifecycle.active_task));
  console.log(JSON.stringify(contextPacket(repo, state, task, positive(parsed.options.bytes || 12000, 'bytes'))));
}
...
Usage:
  ...
  genesis context|next <repo> [task-id] [--bytes 12000] [--id RECORD]
...
function dispatch(command, parsed, raw) {
  if (command === 'context' || command === 'next') return commandContext(parsed);
