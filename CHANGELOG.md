...
- Add `genesis query`: `search`, `defines`, `callers`, `callees`, `impact`, `neighbours` and `path` over the index, with `--json` for machine use. References resolve as a node id, file path, bare symbol name or `path#name`, and ambiguity is reported rather than resolved silently.
