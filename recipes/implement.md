...
Check current phase, scope, dependencies, authorization and blockers. Inspect only the affected code and full records needed for a decision. Implement the active slice; keep formatting/build mutations before verification. Record new facts and corrections in repository memory. Stop on a conflicting requirement or uncertain external side effect.
